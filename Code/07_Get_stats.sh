...
# 3. Find more information about the features, at least all that the Prokka file says regarding ie function (script 072_Prokka_stats.sh)
...
do
  ...
  grep -v "__" $SAMPLE | grep "XF" | awk '{print $NF}' | uniq -c |\
  sed 's/["XF:Z:"]//g' | sed -e 's/^[ \t]*//' \
  > $OUTPUT_FOLDER/Bin_19_test_stats_withoutXF.txt
done
